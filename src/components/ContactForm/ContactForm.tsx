...
export default function ContactForm() {
    const t = useTranslations('ContactForm'); // Specify the translation namespace
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        createContactForm(contactFormData);
        window.alert(t('submissionReceived')); // Use translated alert message
        setContactFormData({
            name: '',
            email: '',
            subject: '',
    ...
    return (
        <div className="flex flex-col">
            <h1 className="text-2xl md:text-3xl xl:text-4xl font-bold mb-4">{t('title')}</h1>
            <p className="text-sm md:text-base xl:text-lg text-justify mb-2 md:mb-4">
                {t('description')}
            </p>
            <form className="flex flex-col gap-3 md:gap-4 lg:gap-5" onSubmit={handleSubmit}>
                <div className="flex flex-col md:flex-row md:space-x-4">
                    <div className="flex flex-col flex-1 gap-1 md:gap-2">
                        <label htmlFor="name" className="text-sm md:text-base xl:text-lg p-1 md:p-2">
                            {t('name')}
                        </label>
                        <input
                            type="text"
                            ...
                            id="name"
                            autoComplete="name"
                            className="p-2 rounded-custom bg-slate-100 text-background"
                            value={contactFormData.name}
                            onChange={handleContactFormChange}
                        />
                    </div>
                    <div className="flex flex-col flex-1 gap-1 md:gap-2 mt-3 md:mt-0">
                        <label htmlFor="email" className="text-sm md:text-base xl:text-lg p-1 md:p-2">
                            {t('email')}
                        </label>
                        <input
                            type="email"
                ...
                <div className="flex flex-col gap-1 md:gap-2">
                    <label htmlFor="subject" className="text-sm md:text-base xl:text-lg p-1 md:p-2">
                        {t('subject')}
                    </label>
                    <input
                        name="subject"
                ...
                <div className="flex flex-col gap-1 md:gap-2">
                    <label htmlFor="message" className="text-sm md:text-base xl:text-lg p-1 md:p-2">
                        {t('message')}
                    </label>
                    <textarea
                        name="message"
                        id="message"
                        className="p-2 rounded-custom bg-slate-100 resize-none h-20 md:h-16 text-background"
                ...
                <div className="flex justify-center mt-2 md:mt-4">
                    <input
                        type="submit"
                        value={t('send')}
                        className="bg-gradient-to-r from-bStart to-bEnd px-8 md:px-10 py-2 rounded-custom font-bold text-sm md:text-base xl:text-lg hover:opacity-90 cursor-pointer"
                    />
                </div>
            </form>
        </div>
